...
/* Jianchao.Shi@BSP.CHG.Basic, 2017/06/19, sjc Add to rerun apsd */
	...
	if(apsd_result->pst != POWER_SUPPLY_TYPE_USB_CDP){
...
static int __smblib_set_prop_pd_active(struct smb_charger *chg, bool pd_active)
{
	...
	if (chg->pd_active) {
		smblib_err(chg, "chg->pd_active=%d\n", chg->pd_active);
		chg->real_charger_type = POWER_SUPPLY_TYPE_USB_PD;
...
static void smblib_handle_apsd_done(struct smb_charger *chg, bool rising)
{
	...
	case FLOAT_CHARGER_BIT:
		...
		break;
	case DCP_CHARGER_BIT:
		vote(chg->pd_disallowed_votable_indirect, HVDCP_TIMEOUT_VOTER, false, 0);
...
/* Jianchao.Shi@BSP.CHG.Basic, 2017/06/19, sjc Modify to rerun apsd */
		...
		if (reg_value & (CDP_CHARGER_BIT | FLOAT_CHARGER_BIT | SDP_CHARGER_BIT)) {
			chg->uusb_apsd_rerun_done = true;
			smblib_rerun_apsd(chg);
			return IRQ_HANDLED;
		}
...
static int oppo_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop() && dischg_flag == false) {
		...
		if ((USB_57C <= chip->usb_temp_r && chip->usb_temp_r < USB_100C)
			...
			count = 0;
			msleep(delay);
			wait_event_interruptible(oppo_usbtemp_wq, qpnp_get_prop_charger_voltage_now() > 3000);
		}
	}
...
#endif
/*zmlin@BSP.CHG.Basic 2019/11/04 mod for cts*/
...
extern struct usbpd *pd_lobal;
extern int usbpd_select_pdo(struct usbpd *pd, int pdo, int uv, int ua);
static int smbchg_pd_setup(void)
{
    struct usbpd *pd = pd_lobal;

    usbpd_select_pdo(pd, 1, 5000000, 3000000);

    return 0;
}

static bool smbchg_get_pd_type(void)
{
    struct oppo_chg_chip *chip = g_oppo_chip;
...
#ifdef VENDOR_EDIT /* wangsen@BSP.POWER.Basic 2019/11/07 add for pd charge */
	.oppo_chg_pd_setup = smbchg_pd_setup,
	.oppo_chg_get_pd_type = smbchg_get_pd_type,
#endif /*VENDOR_EDIT*/
