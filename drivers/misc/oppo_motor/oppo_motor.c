...
static void position_detect_work(struct work_struct *work)
{
	...
	while (chip->motor_started) {
		...
		if (hall0_val > chip->cali_data.down_retard_hall0) {
			if ((chip->move_state == MOTOR_DOWNWARD_ING) && (speed_down == 0)) {
				speed_down = 1;
				oppo_motor_change_speed(&chip->pwm_param.run.speed_down);
				usleep_range(5000, 5000);
				if (chip->motor_retard_offset_support && (hall0_val >= chip->cali_data.down_retard_hall0)
					&& (hall0_val < (chip->cali_data.down_retard_hall0 + chip->stop.offset[1]))) {
					delay_cycle = chip->pwm_param.run.down_brake_delay + 5000;
				} else {
		...
		if (hall0_val < chip->cali_data.irq_pos_hall0 && hall1_val < chip->cali_data.irq_pos_hall1) {
			 ...
			 */
			if ((chip->stop.offset[0] > 0) && (chip->stop.offset[1] > 0)) {
				hall0_cali_diff = chip->run_check_hall0 - chip->cali_data.down_retard_hall0 - chip->stop.cali_retard_offset[1];
				if ((hall0_cali_diff > chip->stop.hall_max_thrd[1]) && (chip->move_state == MOTOR_DOWNWARD_ING)) {
					cur_down_offset = chip->stop.offset[1] + hall0_cali_diff;
					if (hall0_val > (chip->cali_data.down_retard_hall0 + cur_down_offset)) {
						stop_retry = 1;
				...
				} else if (hall0_val > (chip->cali_data.down_retard_hall0 + chip->stop.offset[1])) {
		...
		if ((chip->motor_down_stop_support) && speed_down && (chip->move_state == MOTOR_DOWNWARD_ING)) {
			if (hall0_val >= (chip->cali_data.down_retard_hall0 + chip->stop.offset[1])) {
